...
func (s EDPComponentService) GetEDPComponents() ([]*query.EDPComponent, error) {
	...
	for i, v := range c {
		modifyPlatformLinks(v.Url, c[i], beego.AppConfig.String("projectMaskUrl"))
	}

	return c, nil
}

func modifyPlatformLinks(url string, c *query.EDPComponent, maskUrl string) {
	replacer := strings.NewReplacer("{namespace}", context.Namespace)
	c.Url = fmt.Sprintf("%v" + replacer.Replace(maskUrl), url)
}

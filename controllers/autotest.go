...
func (c *AutotestsController) CreateAutotests() {
	...
	if errMsg != nil {
		...
		c.Redirect(context.BasePath+"/admin/edp/autotest/create", 302)
		return
	}
	...
	flash.Store(&c.Controller)
	c.Redirect(fmt.Sprintf(context.BasePath+"/admin/edp/autotest/overview?%s=%s#codebaseSuccessModal", paramWaitingForCodebase, codebase.Name), 302)
}

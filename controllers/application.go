...
func (c *ApplicationController) CreateApplication() {
	...
	if errMsg != nil {
		...
		c.Redirect(context.BasePath+"/admin/edp/application/create", 302)
		return
	}
	...
	flash.Store(&c.Controller)
	c.Redirect(fmt.Sprintf(context.BasePath+"/admin/edp/application/overview?%s=%s#codebaseSuccessModal", paramWaitingForCodebase, codebase.Name), 302)
}

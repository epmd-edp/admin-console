...
func (c *CDPipelineController) UpdateCDPipeline() {
	...
	if errMsg != nil {
		...
		c.Redirect(fmt.Sprintf(context.BasePath+"/admin/edp/cd-pipeline/%s/update", pipelineName), 302)
		return
	}
	...
	if err != nil {
		...
		case *models.CDPipelineDoesNotExistError:
			...
			flash.Store(&c.Controller)
			c.Redirect(fmt.Sprintf(context.BasePath+"/admin/edp/cd-pipeline/%s/update", pipelineName), http.StatusFound)
			return
		case *models.NonValidRelatedBranchError:
			...
			flash.Store(&c.Controller)
			c.Redirect(fmt.Sprintf(context.BasePath+"/admin/edp/cd-pipeline/%s/update", pipelineName), http.StatusBadRequest)
			return
		default:
			c.Abort("500")
			return
	...
	c.Data["EDPVersion"] = context.EDPVersion
	c.Data["Username"] = c.Ctx.Input.Session("username")
	c.Redirect(context.BasePath+"/admin/edp/cd-pipeline/overview#cdPipelineEditSuccessModal", 302)
}
...
func (c *CDPipelineController) CreateCDPipeline() {
	...
	if errMsg != nil {
		...
		flash.Error(errMsg.Message)
		flash.Store(&c.Controller)
		c.Redirect(context.BasePath+"/admin/edp/cd-pipeline/create", 302)
		return
	}
	...
	if pipelineErr != nil {
		...
		case *models.CDPipelineExistsError:
			...
			flash.Store(&c.Controller)
			c.Redirect(context.BasePath+"/admin/edp/cd-pipeline/create", http.StatusFound)
			return
		case *models.NonValidRelatedBranchError:
			flash.Error(fmt.Sprintf("one or more applications have non valid branches: %v", cdPipelineCreateCommand.Applications))
			flash.Store(&c.Controller)
			c.Redirect(context.BasePath+"/admin/edp/cd-pipeline/create", http.StatusBadRequest)
			return
		default:
			c.Abort("500")
			return
	...
	c.Data["EDPVersion"] = context.EDPVersion
	c.Data["Username"] = c.Ctx.Input.Session("username")
	c.Redirect(fmt.Sprintf(context.BasePath+"/admin/edp/cd-pipeline/overview?%s=%s#cdPipelineSuccessModal", paramWaitingForCdPipeline, pipelineName), 302)
}

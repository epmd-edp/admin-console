...
func (c *BranchController) CreateCodebaseBranch() {
	...
	bns := util.ProcessBranchVersionDashToSlash(branchInfo.Name, branchInfo.Release)
	...
	c.Redirect(fmt.Sprintf("/admin/edp/codebase/%s/overview?%s=%s&%s=%s#branchSuccessModal", appName,
		paramWaitingForBranch, util.ProcessBranchVersionSlashToDash(branchInfo.Name, branchInfo.Release),
		paramReleaseBranch, branchInfo.Release), 302)
}
...
func (c *BranchController) extractCodebaseBranchRequestData() command.CreateCodebaseBranch {
	...
	cb.Name = util.ProcessBranchVersionSlashToDash(cb.Name, cb.Release)

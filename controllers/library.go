...
func (c *LibraryController) Create() {
	...
	if errMsg != nil {
		...
		c.Redirect(context.BasePath+"/admin/edp/library/create", 302)
		return
	}
	...
	flash.Store(&c.Controller)
	c.Redirect(fmt.Sprintf(context.BasePath+"/admin/edp/library/overview?%s=%s#codebaseSuccessModal", paramWaitingForCodebase, codebase.Name), 302)
}
